...
NUM_EMPLOYEES = 2      # кол-во сотрудников
AVG_SUPPORT_TIME = 5   # время обслуживания
CUSTOMER_INTERVAL = 2  # звонки поступают каждые 2 минуты
SIM_TIME = 120         # время моделиирования в минутах

customer_handled = 0   # счетчик
...
class CallCenter:
    ...
    def support(self, customer):
        ...
        print(f"Поддержка завершилась для {customer} на отметке {self.env.now: .2f}")
        
def customer(env, name, call_center):
    global customer_handled
    print(f"Клиент {name} встал в очередь в {env.now: .2f}!")
    with call_center.staff.request() as request:
        yield request
        print(f"Клиент {name} позвонил в {env.now: .2f}")
        yield env.process(call_center.support(name))
        print(f"Клиент {name} завершил звонок в {env.now: .2f}")
        customer_handled += 1
...
print("Клиентов обслужено: " + str(customer_handled))
